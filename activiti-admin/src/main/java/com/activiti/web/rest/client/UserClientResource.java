...
public class UserClientResource extends AbstractClientResource {
	...
	@RequestMapping(value = "/rest/activiti/users/{userId}", method = RequestMethod.GET, produces = "application/json")
	@Timed
	public JsonNode getUser(@PathVariable String userId, HttpServletRequest request) throws BadRequestException {
		
		ServerConfig serverConfig = retrieveServerConfig(request);
		try {
			return clientService.getUser(serverConfig, userId, getRequestParametersWithoutServerId(request));
