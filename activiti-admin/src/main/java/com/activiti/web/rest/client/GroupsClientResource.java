 ...
 */
@RestController
public class GroupsClientResource extends AbstractClientResource {
	
	@Autowired
	protected ActivitiIdentityService clientService;

	@RequestMapping(value = "/rest/activiti/groups", method = RequestMethod.GET, produces = "application/json")
	@Timed
	public JsonNode getGroups(HttpServletRequest request) throws BadRequestException {
		
		ServerConfig serverConfig = retrieveServerConfig(request);
		try {
			return clientService.getGroups(serverConfig, getRequestParametersWithoutServerId(request));
