...
protected  CredentialsProvider credentialsProvider;
	...
	public CloseableHttpClient getHttpClient(String userName, String password) {
        ...
        // create request config to reduce default time outs
        RequestConfig defaultRequestConfig = RequestConfig.custom()
                .setConnectTimeout(getHttpClientConnectionTimeout())
                .setSocketTimeout(getHttpClientSocketTimeout())
	...
	public HttpClientContext createHttpClientContext(HttpUriRequest request){
	...
	public JsonNode executeRequest(HttpUriRequest request, String userName, String password, int expectedStatusCode) {
		...
		try {
            CloseableHttpResponse response = client.execute(request, createHttpClientContext(request));
    ...
    public JsonNode executeDownloadVariableRequest(HttpUriRequest request, HttpServletResponse httpResponse, ServerConfig serverConfig ) {

        ActivitiServiceException exception = null;
        String userName = serverConfig.getUserName();
        String password =  serverConfigService.decrypt(serverConfig.getPassword());
        CloseableHttpClient client = getHttpClient(userName, password);
        try {
            CloseableHttpResponse response = client.execute(request, createHttpClientContext(request));
            try {
    ...
    public String getServerUrl(ServerConfig serverConfig, String uri) {
        return getServerUrl(serverConfig.getContextRoot(), serverConfig.getRestRoot(),
            serverConfig.getServerAddress(), serverConfig.getPort(), serverConfig.getTenantId(), uri);
    }

    public String getServerUrlWithoutTenantId(ServerConfig serverConfig, String uri) {
        return getServerUrl(serverConfig.getContextRoot(), serverConfig.getRestRoot(),
            serverConfig.getServerAddress(), serverConfig.getPort(), null, uri);
    }


	public String getServerUrl(String contextRoot, String restRoot, String serverAddress, Integer port, String tenantId, String uri) {
	...
	public String getServerUrlWithoutTenantId(ServerConfig serverConfig, URIBuilder builder) {
        try {
            return getServerUrlWithoutTenantId(serverConfig, builder.build().toString());
        } catch (URISyntaxException e) {
            throw new ActivitiServiceException("Error while creating Activiti endpoint URL: " + e.getMessage());
        }
    }
    ...
    protected int getHttpClientConnectionTimeout() {
        return env.getProperty("http.client.timeout.connection", Integer.class, 2000);
    }

    protected int getHttpClientSocketTimeout() {
        return env.getProperty("http.client.timeout.socket", Integer.class, 3000);
    }
}
