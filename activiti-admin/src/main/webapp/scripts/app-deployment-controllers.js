...
activitiAdminApp.controller('AppDeploymentController', ['$scope', '$rootScope', '$http', '$timeout','$location','$routeParams', '$modal', '$translate', '$q',
    function ($scope, $rootScope, $http, $timeout, $location, $routeParams, $modal, $translate, $q) {
        ...
        $scope.showAllDecisionTables = function() {
		 ...
		 $scope.deleteApp = function() {
		     ...
		     modalInstance.result.then(function (result) {
		         if (result === true) {
		             $http({method: 'DELETE', url: '/app/rest/activiti/apps/' + $routeParams.appId, params: {serverId: $rootScope.activeServer.id}}).
		...
		$scope.executeWhenReady(function() {
		    // Load deployment
		    $http({method: 'GET', url: '/app/rest/activiti/apps/' + $routeParams.appId, params: {serverId: $rootScope.activeServer.id}}).
  	    	    success(function(data, status, headers, config) {
  	    	        $scope.appDeployment = data;
  	    	        $scope.downloadAppUrl = "app/rest/activiti/apps/export/" + data.deploymentId + "?serverId=" + $rootScope.activeServer.id;

                    // Load process definitions
                    $http({method: 'GET', url: '/app/rest/activiti/apps/process-definitions/' + data.deploymentId, params: {serverId: $rootScope.activeServer.id}}).
                    ...
                    if (data.dmnDeploymentId) {
                        $http({method: 'GET', url: '/app/rest/activiti/apps/decision-tables/' + data.dmnDeploymentId, params: {serverId: $rootScope.activeServer.id}}).
                    ...
                    // Load forms
                    $http({method: 'GET', url: '/app/rest/activiti/forms', params: {serverId: $rootScope.activeServer.id,appId:data.id}}).
...
activitiAdminApp.controller('RedpoloyAppCrtl',
    ['$scope', '$modalInstance', '$http', 'app', function ($scope, $modalInstance, $http, app) {
        ...
        $scope.deployApp = function (targetServerId) {
        ...
        $scope.deployAppAsNewVersion = function (targetServerId, appId) {
            targetServerId = $scope.targetServer.serverConfigs[0].id;
            $scope.status.loading = true;
            $http({method: 'GET', 
