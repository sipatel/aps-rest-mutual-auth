...
activitiAdminApp.controller('JobController', ['$scope', '$rootScope', '$http', '$timeout','$location','$routeParams', '$modal', '$translate',
    function ($scope, $rootScope, $http, $timeout, $location, $routeParams, $modal, $translate) {
		...
		$scope.executeJob = function() {
			$http({method: 'POST', url: '/app/rest/activiti/jobs/' + $scope.job.id, params: {serverId: $rootScope.activeServer.id}}).
		...
		$scope.loadJob = function() {
			$scope.job = {};
			$http({method: 'GET', url: '/app/rest/activiti/jobs/' + $routeParams.jobId, params: {serverId: $rootScope.activeServer.id}}).
	    	success(function(data, status, headers, config) {
	            $scope.job = data;

	            if($scope.job.exceptionMessage) {
	            	// Fetch the full stacktrace, associated with this job
	            	$http({method: 'GET', url: '/app/rest/activiti/jobs/' + $scope.job.id + "/stacktrace", params: {serverId: $rootScope.activeServer.id}}).
	            	success(function(data, status, headers, config) {
...
activitiAdminApp.controller('DeleteModalInstanceCrtl',
    ['$rootScope', '$scope', '$modalInstance', '$http', 'job', function ($rootScope, $scope, $modalInstance, $http, job) {
	  ...
	  $scope.ok = function () {
		  ...
		  $http({method: 'DELETE', url: '/app/rest/activiti/jobs/' + $scope.job.id, params: {serverId: $rootScope.activeServer.id}}).
	    	success(function(data, status, headers, config) {
