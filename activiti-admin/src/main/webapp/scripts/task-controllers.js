...
activitiAdminApp.controller('TaskController', ['$scope', '$rootScope', '$http', '$timeout','$location','$routeParams', '$modal', '$translate', '$q', 'gridConstants',
    function ($scope, $rootScope, $http, $timeout, $location, $routeParams, $modal, $translate, $q, gridConstants) {
		...
		$scope.loadTask = function() {
			...
			// Load task
			$http({method: 'GET', url: '/app/rest/activiti/tasks/' + $routeParams.taskId, params: {serverId: $rootScope.activeServer.id}}).
		...
		$scope.loadRuntimeTask = function() {
			if($scope.task && !$scope.taskCompleted) {
				...
				$http({method: 'GET', url: '/app/rest/activiti/tasks/' + $routeParams.taskId,
					params: {runtime: 'true', serverId: $rootScope.activeServer.id}}).
				success(function(data, status, headers, config) {
    ...
    .then(function(headers) {
        $scope.subTaskGridDefinitions = {
            ...
            columnDefs: [
                ...
                { field: 'assignee', displayName: headers[2], width: 100, cellTemplate: gridConstants.userTemplate},
                { field: 'owner', displayName: headers[3], width: 100, cellTemplate: gridConstants.userTemplate}
            ]
        };
    });
   ...
   .then(function(headers) {
        ...
        $scope.identityLinkGridDefinitions = {
            ...
            columnDefs: [
                ...
                { field: 'groupId', displayName: headers[1], cellTemplate: gridConstants.groupTemplate},
                { field: 'userId', displayName: headers[2], cellTemplate: gridConstants.userTemplate}
            ]
        };
   });
		...
		$scope.loadSubTasks = function() {
			...
			$http({method: 'GET', url: '/app/rest/activiti/tasks/' + $scope.task.id +'/subtasks', params: {serverId: $rootScope.activeServer.id}}).
			success(function(data, status, headers, config) {
				$scope.subTasks = data;
		...
		$scope.loadVariables = function() {
			...
			$http({method: 'GET', url: '/app/rest/activiti/tasks/' + $scope.task.id +'/variables', params: {serverId: $rootScope.activeServer.id}}).
			success(function(data, status, headers, config) {
		...
		$scope.loadIdentityLinks = function() {
			...
			$http({method: 'GET', url: '/app/rest/activiti/tasks/' + $scope.task.id +'/identitylinks', params: {serverId: $rootScope.activeServer.id}}).
			success(function(data, status, headers, config) {
		...
		$scope.showTaskForm = function() {
		    if($scope.task.endTime) {
		        $http({method: 'GET', url: '/app/rest/activiti/task-submitted-form/' + $scope.task.id, params: {serverId: $rootScope.activeServer.id}}).
	            success(function(data, status, headers, config) {
...
activitiAdminApp.controller('CompleteModalInstanceCrtl',
  ...
  $scope.ok = function () {
	  ...
	  $http({method: 'POST', url: '/app/rest/activiti/tasks/' + $scope.task.id, data: {action: 'complete', serverId: $rootScope.activeServer.id}}).
  	  success(function(data, status, headers, config) {
...
activitiAdminApp.controller('ResolveTaskModalInstanceCrtl',
  ...
  $scope.ok = function () {
	  $scope.status.loading = true;
	  $http({method: 'POST', url: '/app/rest/activiti/tasks/' + $scope.task.id, data: {action: 'resolve', serverId: $rootScope.activeServer.id}}).
  	  success(function(data, status, headers, config) {
  		$modalInstance.close(true);
  		$scope.status.loading = false;
      }).error(function(data, status, headers, config) {
...
activitiAdminApp.controller('DeleteTaskModalInstanceCrtl',
  ...
  $scope.ok = function () {
	  $scope.status.loading = true;
	  $http({method: 'DELETE', url: '/app/rest/activiti/tasks/' + $scope.task.id, params: {serverId: $rootScope.activeServer.id}}).
    	success(function(data, status, headers, config) {
    		$modalInstance.close(true);
	  		$scope.status.loading = false;
        }).
...
activitiAdminApp.controller('DelegateModalInstanceCrtl',
  ...
  $scope.ok = function () {
	  $scope.status.loading = true;
	  $http({method: 'POST', url: '/app/rest/activiti/tasks/' + $scope.task.id, data: {action: 'delegate', assignee: $scope.status.user, serverId: $rootScope.activeServer.id}}).
  	  success(function(data, status, headers, config) {
...
activitiAdminApp.controller('AssignModalInstanceCrtl',
    ['$rootScope', '$scope', '$modalInstance', '$http', 'task', function ($rootScope, $scope, $modalInstance, $http, task) {
	  ...
	  $scope.ok = function () {
		  ...
		  $http({method: 'PUT', url: '/app/rest/activiti/tasks/' + $scope.task.id, data: {assignee: rawUserValue, serverId: $rootScope.activeServer.id}}).
	  	  success(function(data, status, headers, config) {
...
activitiAdminApp.controller('EditTaskModalInstanceCrtl',
    ['$rootScope', '$scope', '$modalInstance', '$http', 'task', function ($rootScope, $scope, $modalInstance, $http, task) {
	  ...
	  $scope.ok = function () {
		  $scope.status.loading = true;
		  $scope.model.serverId = $rootScope.activeServer.id;
