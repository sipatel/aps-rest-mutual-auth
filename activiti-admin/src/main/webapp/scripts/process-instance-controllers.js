...
activitiAdminApp.controller('ProcessInstanceController', ['$scope', '$rootScope', '$http', '$timeout','$location','$routeParams', '$modal', '$translate', '$q', 'gridConstants',
    function ($scope, $rootScope, $http, $timeout, $location, $routeParams, $modal, $translate, $q, gridConstants) {
		...
		$scope.loadProcessDefinition = function() {
		    // Load definition
		    $http({method: 'GET', url: '/app/rest/activiti/process-definitions/' + $scope.process.processDefinitionId, params: {serverId: $rootScope.activeServer.id}}).
		...
		$scope.loadProcessInstance = function() {
			...
			// Load process
			$http({method: 'GET', url: '/app/rest/activiti/process-instances/' + $routeParams.processInstanceId, params: {serverId: $rootScope.activeServer.id}}).
			success(function(data, status, headers, config) {
				$scope.process = data;
				...
				if ($rootScope.isBpmSuite) { // only load decision tables and forms if endpoint is a bpmSuite
    ...
    .then(function(headers) {
        $scope.gridTasks = {
            ...
            columnDefs: [
                 ...
                 { field: 'assignee', displayName: headers[2], cellTemplate:gridConstants.userTemplate},
                 { field: 'owner', displayName: headers[3],cellTemplate:gridConstants.userTemplate},
   ...
   .then(function(headers) {
	   var variableValueTemplate = '<div ng-if="row.getProperty(col.field)==\'serializable\' "><div class="ngCellText">' + 
	   '<a  download="{{row.entity.id}}" ng-href="app/rest/activiti/variable-instances/{{row.entity.id}}/data?serverId={{activeServer.id}}"><span class="glyphicon glyphicon-download-alt"></span></a></div></div><div ng-if="row.getProperty(col.field)!==\'serializable\' "><div class="ngCellText">{{row.getProperty(col.field) && row.getProperty(col.field) || "null"}}</div></div>';
    ...
    .then(function(headers) {
	  ...
	  $q.all([$translate('SUBMITTED-FORM.HEADER.ID'),
		...
		$scope.loadTasks = function() {
			...
			$http({method: 'GET', url: '/app/rest/activiti/process-instances/' + $scope.process.id +'/tasks', params: {serverId: $rootScope.activeServer.id}}).
			success(function(data, status, headers, config) {
				$scope.tasks = data;
		...
		$scope.loadVariables = function() {
			...
			$http({method: 'GET', url: '/app/rest/activiti/process-instances/' + $scope.process.id +'/variables', params: {serverId: $rootScope.activeServer.id}}).
			    success(function(data, status, headers, config) {
			        $scope.variables = data;
			        $timeout(function(){
				        angular.forEach($scope.variables.data, function (value, index) {
				        	   if($scope.variables.data[index].variable.type == 'serializable'){
				        		   $.extend($scope.variables.data[index].variable,{id:$scope.variables.data[index].id,value:"serializable"})
				        	   }
				        	});
				        })
		...
		$scope.loadSubProcesses = function() {
			...
			$http({method: 'GET', url: '/app/rest/activiti/process-instances/' + $scope.process.id +'/subprocesses', params: {serverId: $rootScope.activeServer.id}}).
			success(function(data, status, headers, config) {
		...
		$scope.loadJobs = function() {
			...
			$http({method: 'GET', url: '/app/rest/activiti/process-instances/' + $scope.process.id +'/jobs', params: {serverId: $rootScope.activeServer.id}}).
			success(function(data, status, headers, config) {
				$scope.jobs = data;
        ...
        $scope.loadDecisionTables = function() {
            ...
            $http({method: 'GET', url: '/app/rest/activiti/process-instances/' + $scope.process.id +'/decision-tasks', params: {serverId: $rootScope.activeServer.id}}).
            success(function(data, status, headers, config) {
        ...
        $scope.loadForms = function() {
            // Load forms
            $http({method: 'GET', url: '/app/rest/activiti/process-submitted-forms/' + $scope.process.id, params: {serverId: $rootScope.activeServer.id}}).
            success(function(data, status, headers, config) {
                $scope.forms = data;
...
activitiAdminApp.controller('DeleteProcessModalInstanceCrtl',
  ...
  $scope.ok = function () {
	  ...
	  $http({method: 'POST', url: '/app/rest/activiti/process-instances/' + $scope.process.id,
		  data: dataForPost,
		  params: {serverId: $rootScope.activeServer.id}}).
    	success(function(data, status, headers, config) {
    ...
    $scope.executeUpdateVariable = function() {
        ...
        if ($scope.updateVariable.value !== null || $scope.updateVariable.value !== undefined || $scope.updateVariable.value !== '') {
            ...
            if ($scope.updateVariable.type === 'string') {
            	...
            	|| $scope.newVariable.type === 'datetime') {
...
activitiAdminApp.controller('DeleteVariableCrtl',
    ...
    $scope.deleteVariable = function() {
         $http({method: 'DELETE', url: '/app/rest/activiti/process-instances/' + processInstanceId + '/variables/' + $scope.variable.name, params: {serverId: $rootScope.activeServer.id}}).
            success(function(data, status, headers, config) {
                $modalInstance.close(true);
                $scope.status.loading = false;
            }).
