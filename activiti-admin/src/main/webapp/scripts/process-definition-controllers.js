...
activitiAdminApp.controller('ProcessDefinitionController', ['$scope', '$rootScope', '$http', '$timeout','$location','$routeParams', '$modal', '$translate', '$q', 'gridConstants',
    ...
    $scope.loadProcessInstances = function() {
      ...
      $http({method: 'GET', url: '/app/rest/activiti/process-definitions/' + $scope.definition.id +'/process-instances', params: {serverId: $rootScope.activeServer.id}}).
      success(function(data, status, headers, config) {
        ...
        $scope.loadJobs = function() {
            ...
            $http({method: 'GET', url: '/app/rest/activiti/process-definitions/' + $scope.definition.id +'/jobs', params: {serverId: $rootScope.activeServer.id}}).
            success(function(data, status, headers, config) {
                $scope.jobs = data;
        ...
        function loadStartForm () {
            $scope.jobs = undefined;
            $http({method: 'GET', url: '/app/rest/activiti/process-definition-start-form/' + $scope.definition.id, params: {serverId: $rootScope.activeServer.id}}).
            success(function(data, status, headers, config) {
        ...
        $scope.loadDecisionTables = function() {
            ...
            $http({method: 'GET', url: '/app/rest/activiti/process-definition-decision-tables/' + $scope.definition.id, params: {serverId: $rootScope.activeServer.id}}).
            success(function(data, status, headers, config) {
        ...
        $scope.loadForms = function() {
            // Load forms
            $http({method: 'GET', url: '/app/rest/activiti/process-definition-forms/' + $scope.definition.id, params: {serverId: $rootScope.activeServer.id}}).
            success(function(data, status, headers, config) {
                $scope.forms = data;
		...
		$scope.executeWhenReady(function() {
		    // Load definition
		    $http({method: 'GET', url: '/app/rest/activiti/process-definitions/' + $routeParams.definitionId, params: {serverId: $rootScope.activeServer.id}}).
		    success(function(data, status, headers, config) {
		        $scope.definition = data;
		        $scope.loadProcessInstances();
		        $scope.loadJobs();
		        if ($rootScope.isBpmSuite) { // only load decision tables and form if endpoint is a bpmSuite
