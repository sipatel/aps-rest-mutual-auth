    ...
    function ($scope, $rootScope, $http, $timeout, $location, $routeParams, $modal, $translate, $q) {
		...
		$scope.deleteDeployment = function() {
            ...
            modalInstance.result.then(function (result) {
                if (result === true) {
                    $http({method: 'DELETE', url: '/app/rest/activiti/deployments/' + $routeParams.deploymentId, params: {serverId: $rootScope.activeServer.id, cascade: true}}).
		...
		$scope.executeWhenReady(function() {
		    // Load deployment
		    $http({method: 'GET', url: '/app/rest/activiti/deployments/' + $routeParams.deploymentId, params: {serverId: $rootScope.activeServer.id}}).
  	    	    success(function(data, status, headers, config) {
  	    	        $scope.deployment = data;
  	    	        
  	    	        if ($rootScope.isBpmSuite) { // only load app if endpoint is a bpmSuite
  	    	            $http({method: 'GET', url: '/app/rest/activiti/app?deploymentId=' + $routeParams.deploymentId, params: {serverId: $rootScope.activeServer.id}}).
		    ...
		    $http({method: 'GET', url: '/app/rest/activiti/process-definitions?deploymentId=' + $routeParams.deploymentId, params: {serverId: $rootScope.activeServer.id}}).
  	    	    success(function(data, status, headers, config) {
